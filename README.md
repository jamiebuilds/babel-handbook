...
The method signature for a builder is simple, the name is the same as the node
type you want to build, but with the first letter lowercase. For example if you
wanted to build a `MemberExpression` you would use `t.memberExpression(...)`.

The arguments of these builders is decided by the node definition. There's some
...
If you look at the `builder` property, you can see the arguments that will be
passed to the builder method (`t.memberExpression`).

```js
builder: ["object", "property", "computed"],
```

Below that you can see the validation for it. We can see that `object` needs to
be an `Expression`, `property` either needs to be an `Expression` or an
`Identifier` depending on if the member expression is `computed` or not and
`computed` is simply a boolean that defaults to `false`.
...
Well if we look at the definitions again we see an `aliases` property.
