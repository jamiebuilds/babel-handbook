...
> **注意：** 这里同样应该使用声明或者一个声明数组。 这个使用了在用多个节点替换一个节点[Replacing a node with multiple nodes](#replacing-a-node-with-multiple-nodes)中提到的相同的启发式算法。
> 
...
<p>这些构建器的参数由节点定义决定。 有一些还在进行中的工作，用来生成易于阅读的文件定义，但现在他们都可以在<a href="https://github.com/babel/babel/tree/master/packages/babel-types/src/definitions">此处</a>找到。</p>
