...
访问者是一个用于 AST 遍历的跨语言的模式。简单的说它们就是一个对象，定义了用于在一个树状结构中获取具体节点的方法。这么说有些抽象，所以让我们来看一个例子。
...
编写转换时，通常需要构建一些要插入的节点进入 AST。如前所述，您可以使用 [babel-types](https://github.com/jamiebuilds/babel-handbook/blob/master/translations/en/plugin-handbook.md#babel-types) 包中的 [builder](#builders) 方法。

构建器的方法名称就是您想要的节点类型的名称，除了第一个字母小写。例如，如果您想建立一个 `MemberExpression` 您可以使用 `t.memberExpression(...)`。

这些构建器的参数由节点定义决定。生成易于阅读的定义文档的工作正在进行中，但现在他们都可以在[此处](https://github.com/babel/babel/tree/master/packages/babel-types/src/definitions)找到。
...
你可以看到带有 `fields` 对象的 builder 参数的验证。
...
可以看到，`object` 需要是一个 `Expression`，`property` 需要是 `Expression` 还是 `Identifier` 由 member expression 是否是 `computed` 决定，而 `computed`  默认为 `false`。

所以我们可以如此来构建一个 `MemberExpression`： 
...
然而，为什么我们说 `object` 是一个 `Expression` 时 `Identifier` 才是有效的？

如果我们看 `Identifier` 的定义我们可以看到它有一个 `aliases` 属性，表明它本身就是一个表达式。

```js
aliases: ["Expression", "LVal"],
```

所以既然 `MemberExpression` 是 `Expression` 的一种，我们就可以将它设为另外一个 `MemberExpression` 的 `object`：
...
你没什么可能能记住每一个节点的 builder 方法签名，所以你应该花些时间理解他们是如何从节点的定义当中生成的。

你可以在[这里](https://github.com/babel/babel/tree/master/packages/babel-types/src/definitions)找到所有的定义，文档在[这里](https://github.com/babel/babel/blob/master/doc/ast/spec.md)。
...
您可以将它作为状态传递给 `traverse()` 方法，并且在 visitor 的 `this` 中有权访问它。
...
我们忽略了类可以嵌套的事实而使用遍历，上面我们也会得到一个嵌套的 `constructor`：
...
有几种主要的方法来测试 babel 插件：快照测试，AST 测试和执行测试。对于这个例子，我们将使用 [jest](http://facebook.github.io/jest/)，因为它支持盒外快照测试。我们在这里创建的示例是托管在这个 [repo](https://github.com/brigand/babel-plugin-testing-example)。 
...
对于将来的更新，请跟随 [@thejameskyle](https://twitter.com/thejameskyle) 和 [@babeljs](https://twitter.com/babeljs) 的 **Twitter**。
