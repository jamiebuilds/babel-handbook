...
> **注意：** 当用多个节点替换一个表达式时，它们必须是statements。 这是因为Babel在更换节点时广泛使用启发式算法，这意味着您可以做一些非常疯狂的转换，否则将会非常冗长。
...
> 注意：这里同样应该使用声明或者一个声明数组。 这个使用了在[用多节点替换单节点](#replacing-a-node-with-multiple-nodes)中提到的相同的启发式算法。 
...
只需在parentPath：`path.parentPath`调用`replaceWith`即可
...
例如，想象一下，我们想要查找`Foo` `ClassDeclaration`中的`constructor` `ClassMethod`.
...
除了快照测试外，我们还可以手动检查AST。 这是一个简单但是脆弱的例子。 对于更多涉及的情况，您可能希望利用`babel-traverse`。 它允许您用`visitor`键指定一个对象，就像您使用插件本身。
